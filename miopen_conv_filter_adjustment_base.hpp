...
public:
    ...
    void propagate_strides(int *strides, const int *dims,
            std::initializer_list<int> perm) const {
                std::cout<<"filter_adj_base.hpp: "<<__LINE__<<" : "<<__func__<<std::endl;
                std::cout<<"strides[0]= "<<strides[0]<<" "<<"strides[1]= "<<strides[1]<<" "<<"strides[2]= "<<strides[2]<<" "<<"strides[3]= "<<strides[3]<<" "<<"strides[4]= "<<strides[4]<<std::endl;
        int prev_p = -1;
        for (auto p : perm) {
            std::cout<<"I- "<<": strides[prev_p]= "<<strides[prev_p]<<"  "<<"dims[prev_p]= "<<dims[prev_p]<<"  "<<"prev_p= "<<prev_p<<std::endl;
            strides[p] = prev_p == -1 ? 1 : strides[prev_p] * dims[prev_p];
            prev_p = p;
            std::cout<<"O- "<<": strides[prev_p]= "<<strides[prev_p]<<"  "<<"dims[prev_p]= "<<dims[prev_p]<<"  "<<"prev_p= "<<prev_p<<std::endl;
        }
        std::cout<<"strides[0]= "<<strides[0]<<" "<<"strides[1]= "<<strides[1]<<" "<<"strides[2]= "<<strides[2]<<" "<<"strides[3]= "<<strides[3]<<" "<<"strides[4]= "<<strides[4]<<std::endl;
    }
